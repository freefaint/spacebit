  ...
  return (
    <div ref={currentLevel} style={{ display: 'flex', alignItems: 'center', flexGrow: 1, position: 'relative' }}>
              ...
              style={{
                ...
                top:
                  currentTop <= parentTop
                    ? currentStep.current!.clientHeight / 2 + 'px'
                    : currentStep.current!.clientHeight / 2 - Math.max(Math.abs(parentTop - currentTop), 5) + 'px',
              }}
            >
              <Arrow
                up={currentTop < parentTop}
                width={128}
                height={Math.max(Math.abs(parentTop - currentTop), 5)}
                color="#f00"
              />
            </div>
          )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', marginLeft: '10rem' }}>
        {paths?.map((step, j) => (
          <StepBlock {...step} parentTop={currentStep.current?.offsetTop} />
        ))}
      </div>
    </div>
  );
    ...
    >
      <div
        style={{
          ...
          width: '0.75rem',
          height: '0.75rem',
          backgroundColor: value === TrafficLightValue.Danger ? '#a00' : '#ccc',
        }}
      />
      <div
        style={{
          ...
          width: '0.75rem',
          height: '0.75rem',
          backgroundColor: value === TrafficLightValue.Warning ? '#ca0' : '#ccc',
          margin: '0 0.25rem',
        }}
      />
      <div
        style={{
          ...
          width: '0.75rem',
          height: '0.75rem',
          backgroundColor: value === TrafficLightValue.Success ? '#080' : '#ccc',
        }}
      />
    </div>
  );
